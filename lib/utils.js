...
/**
 * Replaces typical characters which seperate a number.
 * @param {string} text input text
 * @returns {string} string without the number
 */
function removeSeparatorChars(text){
    text_new = text.replace(/,|;|\.|-|\(|\)|[|]|%/g,'')
    return text_new
}
 ...
 * @param   {boolean} specialmode replace <br> with space to have better seperation in result
 * @returns {string} text as non html readable text
 */
function htmlText2Text(htmlText, specialmode=false){
    if(specialmode==true){
