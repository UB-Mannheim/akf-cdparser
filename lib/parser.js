...
module.exports = class CdHtmlParser {
                ...
                fn: (lines, i, ret) => {
                    ...
                    try{
                        ...
                        for (var idx = 0; idx < addressLinearr.length; idx++){
                            ...
                            const addressName = addressLines.split('</b>').trimAll()
                            var type = "Sitz"
                            ...
                            for (var idxx = 0; idxx < length; idxx++){
                                if (addressParts[idxx].match("PLZ")){
                                    ...
                                    objTemp.lcustomerzip = addressParts[idxx].split(' ')[1];
                                    objTemp.lcustomercity = addressParts[idxx].split(' ').splice(2,addressParts[idxx].split(' ').length);
                                    addressParts = addressParts.slice(idxx+1, length);
                                    break;
                                }
                            }
                            const SecaddressPart = addressParts.filter(Boolean)
                            const [street_with_number, city_with_zip] = SecaddressPart.shift().split(',').trimAll()
                            objTemp.street = street_with_number.split(' ').splice(0, street_with_number.split(' ').length-1);
                            if (objTemp.street.length>1)
                                objTemp.street = objTemp.street.join(' ')
                            else{objTemp.street = objTemp.street[0]}
                            objTemp.street_number = street_with_number.split(' ')[street_with_number.split(' ').length-1];
                            objTemp.zip = city_with_zip.split(' ')[0];
                            objTemp.city = city_with_zip.split(' ').splice(1,city_with_zip.split(' ').length);
                            ...
                            for (let addressPart of SecaddressPart) {
                                if (addressPart.match('Postfach')) {
                                    ...
                                    if (k.splitAtFirst(' ').length > 1){
                                        objTemp.POBox = k.splitAtFirst(' ')[1]
                                    }else{var POBox = k }
                                    if (v.splitAtFirst(' ').length > 1){
                                        objTemp.zipPOBox = v.splitAtFirst(' ')[0]
                                        objTemp.cityPOBox =  v.splitAtFirst(' ')[1]
                                    ...
                                    else if (k.match(/e.mail/))objTemp.email = v.split(/\s*,\s*/)[0]
                                    else if (k.match('Internetseite'))objTemp.email = v.split(/\s*,\s*/)[0]
    ...
    parseFile(filename) {
        ...
        cfw.writeHeaderToCheckFile(filename+"-----------------------------",cfw.config.checkfile_sitz,cfw.config.checkfile_sitz_enabled);
        cfw.writeHeaderToCheckFile(filename+"-----------------------------",cfw.config.checkfile_taetigkeitsgebiet,cfw.config.checkfile_taetigkeitsgebiet_enabled);
        cfw.writeHeaderToCheckFile(filename+"-----------------------------",cfw.config.checkfile_gruendung,cfw.config.checkfile_gruendung_enabled);
        cfw.writeHeaderToCheckFile(filename+"-----------------------------",cfw.config.checkfile_status,cfw.config.checkfile_status_enabled);
        cfw.writeHeaderToCheckFile(filename+"-----------------------------",cfw.config.checkfile_beschaeftigte,cfw.config.checkfile_beschaeftigte_enabled);
        cfw.writeHeaderToCheckFile(filename+"-----------------------------",cfw.config.checkfile_wknentry,cfw.config.checkfile_wknentry_enabled);
        cfw.writeHeaderToCheckFile(filename+"-----------------------------",cfw.config.checkfile_name,cfw.config.checkfile_name_enabled);
