...
module.exports = class CdHtmlParser {
    ...
    constructor() {
        this._parseFunctions = [
            ...
            },
            {
                ...
                match: 'b>Ausgewählte Beteiligungen|b> Ausgewählte Beteiligungen|b>Mehrheitsbeteiligungen|b> Mehrheitsbeteiligungen', //Not done yet: wird nicht als tabelle behandelt, weil das html fehlerhaft ist
                fn: (lines, i, ret) => {
                    var originline = i;      
            ...
            {
                match: 'b>Mehrheitsbeteiligungen', 
                fn: (lines, i, ret) => {
                    return;
                    var originline = i;      
                    i=i+1; //Start of with one line further
    ...
    doLineSegmentAnalysis(dataToParse,identifier){
        var printOptions = {
            ignoreWhitespaces: true
        }
        var datasetOptions = {
            name:"wesentliche beteiligungen",
            zumsteinVector: false //use zumsteinVector special configuration  
        }
        lineSegmentAnalyzer.addLineArrayToSet(dataToParse);
        lineSegmentAnalyzer.createLocalDataset(datasetOptions);
        lineSegmentAnalyzer.analyzeDataset();
