        ...
        this._parseFunctions = [
                    ...
                    do{
                        ...
                        if (lines[i].match('ISIN:')){
                            ...
                            if (type != null)
                                type = type[1]
                            i = i+1
                            if (entry_idx == 1 && String(isin).match(String(wkn)) == null){
                                //NEUES OBJ
                                all_wkn_entry.push({type, isin, wkn, nw})
                                type, isin, wkn, nw = null
                                type, isin, wkn, nw = ""
            ...
            },
            {
                ...
                fn: (lines, i, ret) => {
                    ...
                    while (!(lines[i].match('<!-- weitere Sitze -->') || (lines[i].match('<br><center><h5><table><tr>')))) //JS: Das produziert eine Index Out Of Bounds exception wenn das Tag nicht vorhanden ist 
                    ...
                    street_with_number.replace(/(.*?)\s*(\d+)$/, (_, street, street_number) => {
                        returnObjTemp.street = street
                        returnObjTemp.street_number = (street_number)
                    })
                    if (city_with_zip) city_with_zip.replace(/^(\d+)\s*(.*)/, (_, zip, city) => {
                        returnObjTemp.zip = zip
                        returnObjTemp.city = city
                    })
                    for (let addressPart of addressParts) {
                        const [k, v] = addressPart.splitAtFirst(':')
                        if (!k) continue
                        if (k.match('Telefon')) returnObjTemp.phone = v
                        else if (k.match('Telefax')) returnObjTemp.fax = v
                        else if (k.match(/e.mail/)) returnObjTemp.email = v.split(/\s*,\s*/)
                        else if (k.match('Internetseite')) returnObjTemp.www = v.split(/\s*,\s*/)
                        else console.error(`Unkwn addressPart ${k}=${v}`)
                    }
                    cfw.writeToCheckfile(originline,JSON.stringify(addressLines, null, " "),JSON.stringify(returnObjTemp, null, " "), cfw.config.checkfile_sitz ,cfw.config.checkfile_sitz_enabled);                                                                
                    return i
                    }catch(ex){
