                ...
                fn: (lines, i, ret) => {
                    ...
                    try{
                        ...
                        for (var idx = 0; idx < addressLinearr.length; idx++){
                            ...
                            if (addressParts.filter(Boolean).shift().match(',') != null){
                                var asdf = 1
                                objTemp.zip = city_with_zip.split(/\s+/)[0];
                                objTemp.city = city_with_zip.split(/\s+/).splice(1,city_with_zip.split(/\s+/).length);
                            ...
                            for (let addressPart of SecaddressPart) {
                                if (addressPart.match('Postfach')) {
                                    ...
                                    if (k.splitAtFirst(/\s+/).length > 1){
                                        objTemp.POBox = k.splitAtFirst(/\s+/)[1]
                                    }else{var POBox = k }
                                    if (v.splitAtFirst(/\s+/).length > 1){
                                        objTemp.zipPOBox = v.splitAtFirst(/\s+/)[0]
                                        objTemp.cityPOBox =  v.splitAtFirst(/\s+/)[1]
