...
var parser_aktionaer    = require('./parser_aktionaer_eigner');
...
module.exports = class CdHtmlParser {
        ...
        this._parseFunctions = [
            ...
            },
            {
                ...
                fn: (lines, i, ret) => {
                    ...
                    return parser_aktionaer.parse_aktionaer(aktionaerlinesFinal,originline,i,ret);
            ...
            },
            {
                ...
                match: 'b>Ausgewählte Beteiligungen', //Not done yet: wird nicht als tabelle behandelt, weil das html fehlerhaft ist
                fn: (lines, i, ret) => {
                    var originline = i;      
                    var currentline = lines[i];
                    var linesAcc =lines[i-1]; 

                    var linesBeteiligungen =[]; //Lines related to InvestorRelations 
                    linesBeteiligungen.push(linesAcc);
                    do{
                        if(currentline.trim()!=""){
                        ...
                        i++;
                        currentline = lines[i];
                    }while(currentline && currentline!="<br><center><h5><table><tr>") 
                   /*
                    if(currentline){
                        linesBeteiligungen.push(currentline);
                    }
                   ...
                   return;
                    ...
                    cfw.writeHeaderToCheckFile(linesBeteiligungen,cfw.config.checkfile_beteiligungen,cfw.config.checkfile_beteiligungen_enabled);
            ...
            },
            {
                ...
                fn: (lines, i, ret) => {
                    ...
                    do{
                        ...
                        i++;
                        currentline = lines[i];
                        var end = currentline.indexOf("b>Kapitalentwicklung");
                    }while(end==-1) 
                   ...
                   var data =  $("table").parsetable(false,false,true);
                    ...
                    cfw.writeHeaderToCheckFile(data[1],cfw.config.checkfile_beteiligungen,cfw.config.checkfile_beteiligungen_enabled);
                    var returnbet =  parser_aktionaer.parse_beteiligungen(data[1],originline,i);
    ...
    parse(linesStr) {
        ...
        //Just test other table parsing TOBEMOVED 
        const $ = cheerio.load(linesStr);
        var htmlt = $.html; 
        var tbody =$("body");
        cheeriotableparser($);
        var tdata = $("table").parsetable();
