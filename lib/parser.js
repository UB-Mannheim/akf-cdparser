...
var fs                      = require('fs');
var hocrdomjsdom            = require('hocr-dom-jsdom');
var hocrdom                 = require('hocr-dom').HocrDOM;
...
module.exports = class CdHtmlParser {
                ...
                */
                ...
                fn: (lines, i, ret) => {
                    ...
                    try{
                        ...
                        //Array.prototype.push.apply(addressLinearr, lines)    
                        addressLinearr.push(addressLines); //last entry push 
                        addressLines = null;
     ...
     */
    parseFile(filename) {
        console.log("Parser.js parsing .html-file: ",filename);
    ...
    parseFileHocr(filename){
        console.log("Parser.js parsing .hocr-file: ",filename);
        //Note to the callcounter, that another file is in use
        callCounter.switchFile(filename);
        //Write the headers to all checkfiles
        cfw.writeAllFilenameHeaderToAllFiles(filename);
        ...
        var hocrDocument = new hocrdomjsdom().parse(filetext);
        var queryLine = {class:'ocr_line'} 
        var queryWord = {class:'ocrx_word'}
        var hocrLines = hocrdom.queryHocrAll(hocrDocument, queryLine);
        var finalLines = []
        var simpleTexts = [] 
        for(var i = 0; i < hocrLines.length; i++){
            var finalLineObject = {};
            finalLineObject.lineIndex = i;
            var currentLine = hocrLines[i];
            var lineText = currentLine.textContent;
            finalLineObject.lineText = lineText;
            var lineTextFiltered = lineText.replace(/\n/g,"").replace(/\s+/g," ").trim();
            finalLineObject.lineTextFiltered = lineTextFiltered;
            simpleTexts.push(lineTextFiltered)
            var lineBox = currentLine.hocr.bbox;
            finalLineObject.lineBox = lineBox;
            var lineWords = hocrdom.queryHocrAll(currentLine, queryWord);
            finalLineObject.words = []
            for(var x= 0; x<lineWords.length;x++){
                var finalWordObject = {};
                var currentWord = lineWords[x];
                var wordText = currentWord.textContent;
                var wordBox = currentWord.hocr.bbox;
                finalWordObject.wordText = wordText;
                finalWordObject.wordBox = wordBox;
                finalLineObject.words.push(finalWordObject);
            }
            finalLines.push(finalLineObject)
        }
        
        //Call parse function for read file
        return this.parseHocr(finalLines, simpleTexts)
    }
    
    /**
     * @param {object} linesHocr hocr-file content
     */
    parseHocr(linesHocr, simpleTexts) {
        ...
        for (let i = 0; i < linesHocr.length; i++) {     
            var currentLine = linesHocr[i];
            var currentText = currentLine.lineTextFiltered   
            if (this.exportVocabulary && i> start_index){
                 this.doExportVocabulary(currentText)
                 continue
            }
            for (let parseFunction of this._parseFunctions) {               
                ...
                var lineTrim = currentText.trim(); 
                var matchfn = parseFunction.matchHocr
                if(matchfn){
                    console.log("asd");
                }
                //if there is a match for a category
                if (lineTrim.match(matchfn) && !parseFunction.disabled) {
                    //call the parse function and increment i, or take the old i when i is not returned
                    i = parseFunction.fn(simpleTexts, i, ret) || i;  
                    break;
                }
            }
        }
