...
const PRINT_FILENAME ="./LineSegmentAnalysis_PRINTOUT.xlsx"
...
function fillExcelWithData(){
    ...
    */
    ...
    for(var i=0;i<_sectorsHolder.length;i++){  
        var skey = generateSectorKey(i+1,1);
        editExcelCell("Character",worksheet,2,skey);
        skey = generateSectorKey(i+1,2);
        editExcelCell("Occurences",worksheet,2,skey);
        skey = generateSectorKey(i+1,3);
        editExcelCell("Grouped",worksheet,2,skey);
    }
    ...
    for(var i=0;i<_sectorsHolder.length;i++){
        ...
        for(var x=0;x<sortedColumn.length;x++){
            ...
            editExcelCell(currentKey,worksheet,rowIndex,sectorKey);
            ...
            editExcelCell(currentItem.isGroup,worksheet,rowIndex,sectorKey);
...
function editExcelCell(value,worksheet,rowNumber,columnID){
    var row = null;
    row = worksheet.getRow(rowNumber);  //Rows start with two 
    //row.getCell('s1p1').value = 1234;
    row.getCell(columnID).value = value;
    row.commit();
}
...
function writeToExcelFile(){   
    ...
    workbook.xlsx.writeFile(PRINT_FILENAME)
    .then(function() {
        console.log("Data written to ",PRINT_FILENAME);
    });
}
