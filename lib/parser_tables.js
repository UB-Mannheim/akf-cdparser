...
function recognizeTableArrays(linesWithTables){
    ...
    for(var i=0;i<linesWithTables.length;i++){
        ...
        if(currentline.indexOf("<table")!=-1){
            ...
            //Table stops 
            inTable = false; 
            //if(currentInfoObject==null)debugger;
        ...
        if(i==0){
            ...
            for(var y=0;y<itemIndicesGroups.length;y++){
                ...
                for(var x=0;x<currentColumn.length;x++){
                    ...
                    _kursitem = assignIndicesToObject(_kursitem,x,itemIndicesGroups[y],currentItemTextLC);
                }
            ...
            if(i==0){
                ...
                for(var y=0;y<itemIndicesGroups.length;y++){
                    ...
                    for(var x=0;x<currentColumn.length;x++){
                        ...
                        var currentItemTextLC = utils.htmlText2Text(currentItem).trim().toLowerCase();
                        if(!currentItemTextLC) continue;
                        dividentitem = assignIndicesToObject(dividentitem,x,itemIndicesGroups[y],currentItemTextLC);
                    }
                    if(!utils.isObjectEmpty(dividentitem)){
                        returnObject.dividenden.push(dividentitem);
                    }
                }
                */
                    var itemToFill = {};
                    ...
                    if(!utils.isObjectEmpty(itemToFill)){ //also push empty objects ... because of indexing ... clear later
                        if(!itemToFill.jahr){
                            //Special case, the year was defined in the table above this item
                            //content get's assigned to the content from the above item
                            
                            if(returnObject.dividenden[i-1]){
                                for(key in itemToFill){   
                                    if (itemToFill.hasOwnProperty(key)) {     
                                        if(itemToFill[key]){
                                            returnObject.dividenden[i-1][key] = itemToFill[key];
                                        }
                                    }
                                }
                            }else{
                            ...
                            returnObject.dividenden.push(itemToFill);
                        }
                    }else{
                ...
                //for(var y=0;y<itemIndicesGroups.length;y++){
                    ...
                    if(!utils.isObjectEmpty(itemToFill)){ //also push empty objects ... because of indexing ... clear later
                        if(!itemToFill.jahr){
                            ...
                            if(returnObject.kennzahlen[i-1]){
                                for(key in itemToFill){   
                                    if (itemToFill.hasOwnProperty(key)) {     
                                        if(itemToFill[key]){
                                            returnObject.kennzahlen[i-1][key] = itemToFill[key];
                                        }
                                    }
                                }
                            }else{
                            ...
                            //Standard case, there is a year in the item recognized 
                            returnObject.kennzahlen.push(itemToFill);
                        }
                    }else{
                        //Just push empty object, because of indexing .... will be terminated later again
                        returnObject.kennzahlen.push(itemToFill);
