...
function parse_bereinigtekurse(tablesParsed,tablesArr,rest,infos,tableGrundkapitalIndex){
    var tableBereinigteKurse = tablesParsed[tableGrundkapitalIndex];
    ...
    //If the table has a valid header, check the table
    if(tableBereinigteKurse && tableBereinigteKurse[0][0].toLowerCase().indexOf("grundkapital")!=-1){ 
        //parse data
        grundkapital.betrag = utils.htmlText2Text(tableBereinigteKurse[1][0]);

        //If there are additional entries in the table 
        if(tableBereinigteKurse[0] && tableBereinigteKurse[0].length>1){
            for(var i=1;i<tableBereinigteKurse[0].length;i++){
                var tcontent = tableBereinigteKurse[0][i];
                //TODO: this can be less redundant 
                if(!tcontent) continue;
