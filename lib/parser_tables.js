...
function parse_kapitalEntwicklung(tablesParsed,tableKapitalEntw_index){
    ...
    for(var i=0;i<table_kapitalEntwicklung[0].length;i++){
        ...
        if(table_kapitalEntwicklung[1] && table_kapitalEntwicklung[1][i]){
            ...
            var textMostRight = utils.htmlText2Text(table_kapitalEntwicklung[2][i]);
...
function parse_ausgegebenesKapital(tablesParsed,tablesArr,rest,infos,tableGrundkapitalIndex){
    ...
    if(tablesParsed && tablesParsed.length==1){
        ...
        debugger; //Special case: What's going on? 
    }
...
function parse_dividenden(tablesParsed,tablesArr,rest,infos,callnumber){
    ...
    for(var key in rest){
        if(rest.hasOwnProperty(key)){
            ...
            if(rest[key].indexOf("<b>Dividenden</b>")==-1 && rest[key].indexOf("________")==-1){
                var clearBemerkung = utils.htmlText2Text(rest[key]);
            ...
            if(i==0){            
              var currentItemIndices = new itemIndices();
                ...
                for(var x=0;x<currentColumn.length;x++){
                    ...
                    if(currentItemText){
                        var parenthMatch = regLib.matchBetweenParenthesis(currentItemText);
                        if(parenthMatch && parenthMatch.length>0){
                            returnObject.waehrungsinfo.push(currentItemText);
                        }
                    }
...
function recognizeKurseContent(returnObject,item,i){
    ...
    if(item.indexOf("ausgegebenes")!=-1){
