...
function parse_dividenden(tablesParsed,tablesArr,rest,infos){
    ...
    function dividenditem(){
        ...
        this.dividende;
        this.bonus;
        this.sonderausschuettung; 
        this.steuerguthaben;
        this.divscheinnr;
        this.extag;
    };

    //Row indices of which assign values to dividenditem variables, 
    //these are assigned by parsing the first column in the array 
    function itemIndices(){
        this.jahr=-1;
        this.dividende=-1;
    ...
    for(var s=0;s<tablesParsed.length;s++){
        ...
        for(var i=0;i<tableDividenden.length;i++){
            ...
            if(i==0){
                ...
                //The values get assigned to the indices here 
                for(var y=0;y<itemIndicesGroups.length;y++){
                    var _kursitem = new dividenditem();
                    ...
                    if(!utils.isObjectEmpty(_kursitem)){
                        returnObject.dividenden.push(_kursitem);
                    }
                }
            }  
        }
