/**
 * parser_aktionaer_eigner_beteiligungen.js
 * Provides methods for parsing content related to: 
 * Aktionaere,
 * Anteilseigner,
 * Beteiligungen
 */
...
function parse_beteiligungen(tablesArray,tablesParsed,rest,infos){
    ...
    var retBeteiligungen = {
        elemente: [],
        bemerkungen: []
    }   //Holder array for returnvalue 
    ...
    if(rest[0]){
        ...
        if(maybeFirstHeader){            
            var isInParenthesis = false; //Indicates if the header is only parenthesis 
            var parContent = utils.getParenthesisContent(maybeFirstHeader);
            if(parContent && parContent.length>=1){
                var woPar = maybeFirstHeader.replace(parContent[0],"").trim();
                if(!woPar){
            ...
            if(!isInParenthesis){
                ...
                retBeteiligungen.bemerkungen.push(maybeFirstHeader);
            }
        }
    }
        ...
        if(currentInfoObject && nextInfoObject){
            for(var y=currentInfoObject.indexStop+1;y<nextInfoObject.indexStart;y++){
                if(rest[y]){
                    ...
                    if(currentRest){
                        retBeteiligungen.elemente.push(currentBeteiligungsItem);
    ...
    if(currentBeteiligungsItem){ //&& currentBeteiligungsItem.zeilen.length>=1){
        retBeteiligungen.elemente.push(currentBeteiligungsItem);       
    }
...
/**
 * Split the input array entries at breaks and create a final array for parsing.
 * Remove the first entry in the created array. Also removes html-tags and empty elements.
 * 
 * @param {string} linesBeteiligungen 
 */
function prepareBeteiligungen(linesBeteiligungen){
    var beteiligungenModified=""; 

    //If the input array has one element, split at  break and push to linesBR_BSplit
    ...
    //Split array at breaks and remove html tags - create new array
...
/**
 * Parse a non-array beteiligungen, detects headline in the lines.
 * Attaches entries related to headline. Adds the item to returnArray etc...
 * @param {array} linesBeteiligungen - linearray to check upon
 */
function parse_beteiligungen2(linesBeteiligungen){
    var returnArray=[];
    
    /**
     * Class for holding the arrayItem information.
     */
    function beteiligungsitem(){
        this.ueberschrift;
        this.zeilen = [];
    ...
    for(var i=0;i<linesBeteiligungen.length;i++){
        ...
        //If it's an headline push headline, otherwise push row
        if(regLib.doesEndWithColon(currentline.trim())){
            //Remove ':' at end of ueberschrift 
