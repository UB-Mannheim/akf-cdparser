/**
 * parser_organbezuege.js
 * Parser functions for organbezüge related content
 * An item for organbezuege contains always: jahr, organ, bezuege,bemerkung
 */
...
function parseOrganbezuege(organbezuege,originline,i,ret){
    ...
    organbezuege.split(/\s*;\s*/).trimAll().forEach(_ => {      
        ...
        if(oneYearForAll){
            ...
            if(_.indexOf("keine Vergütung")!=-1){
                ...
                if(waehrungmatch!=null){
                    ...
                    bezuege = regLib.removeLastComma(bezuege);
                    if(linesplit[2] && linesplit[2].trim()){
                        bezuege = bezuege+", "+waehrung+" "+ linesplit[2].trim();
                    }
                }else{
        ...
        retOrganbezuege.push(organbezug);
       
    });
    
    return retOrganbezuege;
}
