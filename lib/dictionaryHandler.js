...
function checkIfTitleIsInArray(data,seperator=',',matching='normal'){    
    ...
    /*
    if(data[0].indexOf("Prof. Dr.-Ing. Klaus G. Lederer")!=-1){
    ...
    */
    ...
    for (var i=0;i<data.length;i++) {
        ...
        if(titleInfo.isTitle){
            ...
            dataWithoutTitleAccu = dataWithoutTitleAccu +seperator+ data[i].replace(titleInfo.titleFound,"");
            if(titleInfo.hasRest){
                nameAccu = nameAccu +seperator+ titleInfo.dataWithoutTitle;
            }
            /*
            if(startwithLetter==null ||!titleInfo.hasRest){ 
           ...
           }else if(startwithLetter!=null){
            	//Return non-accumulated title
                returnObject.hasTitle = titleInfo.isTitle;
                returnObject.titleDetected = titleInfo.titleFound;
                if(titleInfo.hasRest){
                    returnObject.hasName= true;
...
function checkIfaDictContentIsInString(data,dict,dict_loaded,options = {matching:"normal"}){
    ...
    for(var i=0;i<dict["rows"].length;i++){
        ...
        /*
        if(entry.indexOf("Vorstandsmitglied d.")!=-1 && data.indexOf("Urbar")!=-1){
            debugger;
        }
        */
