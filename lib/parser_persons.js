...
function checkPersonLine(_,originline,commonfunct_isThere,commonfunct){
    ...
    //Remove the html-rests 
    if(firstName){
        firstName = utils.htmlText2Text(firstName);
    }
    if(lastName){
        lastName = utils.htmlText2Text(lastName);
    }
    if(cityAcc){
        cityAcc = utils.htmlText2Text(cityAcc);
    }
    if(bemerkung){
        bemerkung = utils.htmlText2Text(bemerkung);
    }
...
function parse_investorRelations(investorRelLines,originline,y,ret){
    ...
    //First of all split all lines in array on ; and push on a final array
    for(var i=0;i<investorRelLines.length;i++){
        var currentline = investorRelLines[i];
        //Remove Semicolon for entrys which belong together
        currentline = currentline.replace(/; Fax/g,"  Fax");
        currentline = currentline.replace(/; Telefon/g,"  Telefon");
        currentline = currentline.replace(/; Email/g,"  Email");
        currentline = currentline.replace(/; E-Mail/g,"  E-Mail");
        if(currentline){
            var currentlineSplit = currentline.split(";");
            for(var x=0;x<currentlineSplit.length;x++){
                if(currentlineSplit[x] && currentlineSplit[x].trim()){
                    investorRelLinesFinal.push(currentlineSplit[x].trim());
                }
            }            
        }
    }
    ...
    for(var i=0;i<investorRelLinesFinal.length;i++){
        var name=""; 
        var telefon=""; 
        var fax=""; 
        var funct="";
        ...
        */
        ...
        var functInfo = dictionaryHandler.checkIfaFunctContentIsInString(investorRelLinesFinal[i]); 
        var dataToCheck = functInfo.isTitle?functInfo.dataWithoutTitle:investorRelLinesFinal[i];
        ...
        //Detect Telefon 
        var telefonmatch = regexLib.matchFromStringToString(dataToCheck,"Tel\..*:|Telefon:|Telefon :",",|;|Fax");
        ...
        //Detect Fax 
        var faxmatch = regexLib.matchFromStringToString(dataToCheck,"Fax:|Fax :",",|;|$|Telefon");
        ...
        if(dataToCheckRest.length>=1){
            var dataToCheckRestSplit = dataToCheckRest.split(";");
             Array.prototype.push.apply(restOfLines,dataToCheckRestSplit)
        }
        ...
        for (let k in manager) if (!manager[k]) delete manager[k];
        cfw.writeToCheckfile(originline,investorRelLinesFinal[i],JSON.stringify(manager), cfw.config.checkfile_investorRelations ,cfw.config.checkfile_investorRelations_enabled);                                        
...
function parse_gesellschafter(gesellschafterItems,originline){
    ...
    for(var i=0;i<gesellschafterItems.length;i++){
        ...
        if(currentItem.indexOf("Gesellschafter:<")!=-1){
        ...
        }else if(currentItem.indexOf(">Komplementär")!=-1){
            currentMode = CURRENTMODE.KOMPLEMENTAER;
        }else{
            ...
            switch (currentMode) {
                ...
                case CURRENTMODE.AKTIONAERE:
                    var aktionaerMockArray = [];
                    aktionaerMockArray.push(currentItemText);
                    var aktionaerRes = parser_beteiligungen.parse_aktionaer(aktionaerMockArray,originline,originline);
                    if(!returnObject.aktionaere)returnObject.aktionaere = [];
                    for(var x=0;x<aktionaerRes.length;x++){
                        returnObject.aktionaere.push(aktionaerRes[x]);
                    }
                    break;
                case CURRENTMODE.ANTEILSEIGNER:
                    ...
                    break;
                case CURRENTMODE.KOMPLEMENTAER:
                   
                    var kompsplit = currentItem.split(',');
                    var kompObj={
                        name: kompsplit.length>=1 ? kompsplit[0]:"",
                        ort: kompsplit.length>=2 ? kompsplit[1]:""
                    }
                    if(!returnObject.komplementaer)returnObject.komplementaer = [];
                    returnObject.komplementaer.push(kompObj);
                    break;
                default:
...
/**
 * Parse an array of lines related to a 'Leiter' 
 * this occurs in the match for 'leitung_kommunikation' in parser.js
 * 
 * @param {array} leiterLines - 
 */
function parse_leiter(leiterLines){
    ...
    /**
     * Class Holder for the properties of a leiter
     */
    function leiterItem(){
        this.firstname;
        this.lastname;
    ...
    for(var i=0;i<leiterLines.length;i++){
        ...
        //Get telephone, fax and email and match it. 
        ...
        //Get person details and push them, when found in the leiter item
